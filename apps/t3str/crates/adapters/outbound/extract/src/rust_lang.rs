...
/// directory or with `test` in the filename), then uses tree-sitter to
/// extract functions annotated with `#[test]` or `#[tokio::test]`.
...
///
/// Returns `true` if the file is inside a `tests/` directory or has `test`
/// in its filename.
